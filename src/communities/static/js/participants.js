$(function() {
	...
	var tpl = '<p>{{value}}</p>';
	var member_tpl = '<li class="list-group-item" data-uid="#ID#">' + '<div style="display: inline;line-height: 30px;">' + '#NAME#</div>' + '<button class="del_member pull-right btn btn-danger btn-sm">' + '<i class="fa fa-trash-o"></i> {% trans "Delete" %}' + '</button></li>';
	var guest_tpl = '<li class="list-group-item" data-guest="#GUEST_RAW#">' + '<div style="display: inline;line-height: 30px;">' + '#G_DETAIL#</div>' + '<button class="pull-right btn btn-danger btn-sm">' + '<i class="fa fa-trash-o"></i> {% trans "Delete" %}</button></li>';
